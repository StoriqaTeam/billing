    ...
    match *data {
                        ...
                        .map(|f| {
                            let diesel_type = match &f.ty {
                                Type::Path(p) => p.path
                                    ...
                                    .iter()
                                    .filter_map(|segment| {
                                        let s = segment.ident.to_string().to_lowercase();
                                        match s {
        ...
        Data::Enum(ref data) => {
            ...
            let variants_db: Vec<LitByteStr> = data.variants
                .iter()
                .map(|variant| LitByteStr::new(variant.ident.to_string().to_lowercase().as_bytes(), Span::call_site()))
                .collect();
                ...
                impl FromSqlRow<VarChar, Pg> for #name {
                    fn build_from_row<R: Row<Pg>>(row: &mut R) -> Result<Self, Box<Error + Send + Sync>> {
                        match row.take() {
                            ...
                            Some(v) => Err(format!("Unrecognized enum variant: {:?}", v).to_string().into()),
